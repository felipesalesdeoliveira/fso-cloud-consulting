...
export const metadata: Metadata = {
  title: "Frontend with CDN, S3, TLS and custom domain | FSO Cloud Consulting",
  description: "A production-ready React SPA architecture with private S3, CloudFront, ACM, Route 53 and OAC.",
  alternates: {
...
const items = {
  ...
  validation: ["HTTP redirects to HTTPS with a 301 response.", "Unknown SPA routes return index.html instead of an S3 error.", "Direct access to the S3 origin is denied."],
...
export default function EnglishProjectPage() {
  return <><Header locale="en" /><main className="bg-[#F8FAFD] pt-24 text-slate-900"><section className="border-b border-slate-200 px-6 py-20"><div className="mx-auto max-w-6xl"><Link href="/en/#projects" className="text-sm font-bold text-blue-600">← Back to projects</Link><div className="mt-9 flex gap-3"><span className="rounded-full bg-emerald-100 px-4 py-2 text-xs font-bold text-emerald-700">COMPLETED</span><span className="rounded-full bg-blue-100 px-4 py-2 text-xs font-bold text-blue-700">CLOUD & CDN</span></div><h1 className="mt-7 max-w-5xl text-4xl font-bold tracking-[-0.04em] md:text-6xl">Frontend with CDN, S3, TLS and custom domain</h1><p className="mt-7 max-w-3xl text-xl leading-9 text-slate-600">A secure, scalable architecture for a React SPA with a private origin, global distribution, HTTPS and custom DNS on AWS.</p></div></section>
  ...
  <section className="bg-white px-6 py-20"><div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2"><div><p className="section-label">Validation</p><h2 className="section-title">Verified after deployment</h2><ul className="mt-8 grid gap-4">{items.validation.map(text => <li key={text} className="rounded-2xl border border-emerald-200 p-5 text-slate-700">✓ {text}</li>)}</ul></div><div><p className="section-label">Outcomes</p><h2 className="section-title">A complete delivery for the defined scope</h2><ul className="mt-8 grid gap-4">{items.results.map(text => <li key={text} className="rounded-2xl bg-blue-50 p-5 text-slate-700">{text}</li>)}</ul></div></div></section></main><Footer locale="en" /></>;
}
